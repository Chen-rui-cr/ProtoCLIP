...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #         x_new.append(self.attnpool(x[:,:,i,j].unsqueeze(dim=-1).unsqueeze(dim=-1)))
        # x_new = torch.stack(x_new, dim=-1)
        # x_new = x_new.view(x_new.shape[0], x_new.shape[1], x.shape[-1], x.shape[-2])
        return x, x_pool
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.positional_embedding = nn.Parameter(
            scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width))  # [rid**2+1, width]
